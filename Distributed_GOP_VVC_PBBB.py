	...
	#print >> fid, 'Frame1:    P   1   5       -6.5                      0.2590         0          0          1.0      0            0               0             4                4         1 2 3 4      0   0'
	   ...
	   if cnt in ref_pics_Stitching_array_Distributed:
	      if len(ref_pics_stitch_to_use_Distributed) < num_ref_pics_active_Stitching_Distributed: 
           ...
           if (cnt==1) or (insertedB==3):	
	      GOPLine='Frame' + str(cnt) + ': P '+ str(cnt) +' 0 -6.5 0.2590 0 0 1.0 0 0 0 '+ str(len(ref_pics_Distributed)+0) + ' ' + str(len(ref_pics_Distributed)+0)
	      for cnt1 in range(len(ref_pics_Distributed)):
	         GOPLine=GOPLine+' '+str(int(cnt - ref_pics_Distributed[cnt1]))
              ...
              GOPLine='Frame' + str(cnt) + ': B '+ str(cnt) +' 0 -6.5 0.2590 0 0 1.0 0 0 0 '+ str(len(ref_pics_Distributed)+0) + ' ' + str(len(ref_pics_Distributed)+0)
              for cnt1 in range(len(ref_pics_Distributed)):
                 GOPLine=GOPLine+' '+str(int(cnt - ref_pics_Distributed[cnt1]))
              #GOPLine = GOPLine + ' ' +str(GOP+cnt)

              GOPLine=GOPLine+' '+ str(len(ref_pics_Distributed)+0) + ' ' + str(len(ref_pics_Distributed)+0)
              for cnt1 in range(len(ref_pics_Distributed)):
                 GOPLine=GOPLine+' '+str(int(cnt - ref_pics_Distributed[cnt1]))
              #GOPLine = GOPLine + ' ' +str(GOP+cnt)
              insertedB=insertedB+1
