	   ...
	   if cnt in ref_pics_Stitching_array_Distributed:
	      ...
	      for cnt1 in range(len(ref_pics_Distributed)):
	         GOPLine=GOPLine+' '+str(int(cnt - ref_pics_Distributed[cnt1]))
              ...
              GOPLine='Frame' + str(cnt) + ': B '+ str(cnt) +' 0 -6.5 0.2590 0 0 1.0 0 0 0 '+ str(len(ref_pics_Distributed)+1) + ' ' + str(len(ref_pics_Distributed)+1)
              for cnt1 in range(len(ref_pics_Distributed)):
                 GOPLine=GOPLine+' '+str(int(cnt - ref_pics_Distributed[cnt1]))
              GOPLine = GOPLine + ' ' +str(GOP+cnt)

              GOPLine=GOPLine+' '+ str(len(ref_pics_Distributed)+1) + ' ' + str(len(ref_pics_Distributed)+1)
              for cnt1 in range(len(ref_pics_Distributed)):
                 GOPLine=GOPLine+' '+str(int(cnt - ref_pics_Distributed[cnt1]))
              GOPLine = GOPLine + ' ' +str(GOP+cnt)
